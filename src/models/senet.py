...
class SeNet(nn.Module):
    ...
    def __init__(self, model_name='se_resnext50_32x4d', n_channels=3, n_classes=None, pretrained=False):
        super(SeNet, self).__init__()
        model_encoder = {
            'se_resnext50_32x4d': {
        ...
        if pretrained:
            self.model.load_state_dict(model_zoo.load_url(model_encoder[model_name]['url']))
        ...
        self.model.layer0.conv1 = nn.Conv2d(in_channels=n_channels,
                                                                ...
                                                                bias=conv1.bias)
        if n_channels <= 3:
            self.model.layer0.conv1.weight.data[:, :n_channels, :, :] = conv1.weight.data[:, :n_channels, :, :]
        else:
